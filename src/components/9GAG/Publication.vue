<template>
  <div class="card mb-5 p-5" v-if="login">
    ...
    <p v-if="utilisateur.role !== 'MODERATEUR'" class="alert-info text-center">Vous êtes connecté en tant que {{ this.utilisateur.email_display }}, <a href="/" @click="logout">se déconnecter</a></p>
    ...
    <ul class="liste">
      <li v-for="(value, index) in publications" :key="index">
        <div :id="'publication-' + value.id">
          ...
          <div>
            <img class="avatar" :src="value.utilisateur.image_url" alt="Avatar"> Publié par {{ value.utilisateur.email_display }} <small class="text-muted">le {{ value.date_publication | formatDate }}</small> <span class="badge bg-success">{{ value.date_publication | duration }}</span>
          </div>
          <div class="mt-3">
            <h4>{{ value.description }}</h4>
          </div>
          <div class="mt-2">
            <img class="img-thumbnail" :src="value.image_url" alt="Image publication">
          </div>
